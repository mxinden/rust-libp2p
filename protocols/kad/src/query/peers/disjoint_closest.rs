...
impl DisjointClosestPeersIter {
    ...
    {
        Self::with_config(DisjointClosestPeersIterConfig::default(), target, known_closest_peers)
    }

    /// Creates a new iterator with the given configuration.
    pub fn with_config<I, T>(config: DisjointClosestPeersIterConfig, target: T, known_closest_peers: I) -> Self
    where
        I: IntoIterator<Item = Key<PeerId>>,
        T: Into<KeyBytes> + Clone,
    {
        let iters = split_inputs_per_iter(&config, known_closest_peers)
            .into_iter()
            ...
            loop {
                match iter.next(now) {
                    PeersIterState::Waiting(None) => {
                        match state {
                            PeersIterState::Waiting(None) => {},
                            PeersIterState::WaitingAtCapacity => state = PeersIterState::Waiting(None),
    ...
    // TODO: Collects all Iterators as into a Vec and again returns an Iterator. Can we do better?
...
fn split_inputs_per_iter<I>(config: &DisjointClosestPeersIterConfig, peers: I) -> Vec<(ClosestPeersIterConfig, Vec<Key<PeerId>>)> where I: IntoIterator<Item = Key<PeerId>> {
...
mod tests {
    ...
    #[test]
    fn split_inputs_per_iter_quickcheck() {
        fn prop(config: DisjointClosestPeersIterConfig, peers: PeerVec) -> TestResult {
            ...
            let mut iters = split_inputs_per_iter(&config, peers.0.clone());

            assert_eq!(
                config.parallelism,
                iters.iter().fold(0, |acc, (config, _)| {
                    acc + config.parallelism
                }),
            );
            ...
            assert_eq!(
                config.num_results,
                iters.iter().fold(0, |acc, (config, _)| {
                    acc + config.num_results
                }),
            );

            assert_eq!(
                peers.0.len(),
                iters.iter().fold(0, |acc, (_ , peers)| {
                    acc + peers.len()
                }),
            );

            iters.sort_by(|(a_config, _), (b_config, _)| a_config.parallelism.cmp(&b_config.parallelism));
            assert!(iters[iters.len() - 1].0.parallelism - iters[0].0.parallelism <= 1);

            iters.sort_by(|(a_config, _), (b_config, _)| a_config.num_results.cmp(&b_config.num_results));
